package com.tamu.tio.websocket.server;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.web.bind.annotation.ExceptionHandler;
import org.tio.core.ChannelContext;
import org.tio.core.Tio;
import org.tio.http.common.HttpRequest;
import org.tio.http.common.HttpResponse;
import org.tio.utils.lock.SetWithLock;
import org.tio.websocket.common.WsRequest;
import org.tio.websocket.common.WsResponse;
import org.tio.websocket.common.WsSessionContext;
import org.tio.websocket.server.handler.IWsMsgHandler;
import org.tio.websocket.starter.EnableTioWebSocketServer;

import java.text.SimpleDateFormat;
import java.util.Objects;

/**
 * @author yang
 * 2020年5月13日11:14:12
 */

public class ShowcaseWsMsgHandler implements IWsMsgHandler {
	private static Logger log = LoggerFactory.getLogger(ShowcaseWsMsgHandler.class);

	public static final ShowcaseWsMsgHandler me = new ShowcaseWsMsgHandler();

	private SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

	private ShowcaseWsMsgHandler() {

	}

	/**
	 * <li>对httpResponse参数进行补充并返回，如果返回null表示不想和对方建立连接，框架会断开连接，如果返回非null，框架会把这个对象发送给对方</li>
	 * <li>注：请不要在这个方法中向对方发送任何消息，因为这个时候握手还没完成，发消息会导致协议交互失败。</li>
	 * <li>对于大部分业务，该方法只需要一行代码：return httpResponse;</li>
	 * @param
	 * @param httpResponse
	 * @param channelContext
	 * @return
	 * @throws Exception
	 * @author yang
	 */
	@Override
	public HttpResponse handshake(HttpRequest request, HttpResponse httpResponse, ChannelContext channelContext) throws Exception {
		String clientip = request.getClientIp();
		String myname = request.getParam("name");
		String groupIds = request.getParam("groupId");
		//不可为null
		if(clientip==null || "".equals(clientip) || myname==null || "".equals(myname) ||groupIds==null || "".equals(groupIds)){
			log.info("ws握手失败！");
			return null;
		}
		//绑定群组
		//Tio.bindGroup(channelContext.tioConfig, myname,groupIds);

		//Tio.bindUser(channelContext, myname);
//		channelContext.setUserid(myname);
		log.info("收到来自{}的ws握手包\r\n{}", clientip, request.toString());
		return httpResponse;
	}

	/**
	 * 握手成功后触发该方法
	 * @param httpRequest
	 * @param httpResponse
	 * @param channelContext
	 * @throws Exception
	 * @author yang
	 */
	@Override
	public void onAfterHandshaked(HttpRequest httpRequest, HttpResponse httpResponse, ChannelContext channelContext) throws Exception {
		//接收握手包的参数
		String name = httpRequest.getParam("name");
		String groupIds = httpRequest.getParam("groupId");

		//绑定到群组，后面会有群发
		Tio.bindGroup(channelContext, groupIds);
		int count = Tio.getAll(channelContext.tioConfig).getObj().size();

		String msg = "{name:'admin',message:'" + name+"于"+simpleDateFormat.format(System.currentTimeMillis())+ " 进来了，共【" + count + "】人在线" + "'}";
		log.info("message:", msg);
		//用tio-websocket，服务器发送到客户端的Packet都是WsResponse
		WsResponse wsResponse = WsResponse.fromText(msg, ShowcaseServerConfig.CHARSET);
		//群发
		Tio.sendToGroup(channelContext.tioConfig, groupIds, wsResponse);





		//Tio.bindGroup(channelContext, Const.GROUP_ID);
		// int count = Tio.getAll(channelContext.tioConfig).getObj().size();
		//
		// String msg = "{name:'admin',message:'" + channelContext.userid + " 进来了，共【" + count + "】人在线" + "'}";
		// //用tio-websocket，服务器发送到客户端的Packet都是WsResponse
		// WsResponse wsResponse = WsResponse.fromText(msg, ShowcaseServerConfig.CHARSET);
		// //群发
		// Tio.sendToGroup(channelContext.tioConfig, Const.GROUP_ID, wsResponse);
	}

	/**
	 * <li>当收到Opcode.BINARY消息时，执行该方法。也就是说如何你的ws是基于BINARY传输的，就会走到这个方法</li>
	 * @param wsRequest
	 * @param bytes
	 * @param channelContext
	 * @return 可以是WsResponse、byte[]、ByteBuffer、String或null，如果是null，框架不会回消息
	 * @throws Exception
	 * @author yang
	 */
	@Override
	public Object onBytes(WsRequest wsRequest, byte[] bytes, ChannelContext channelContext) throws Exception {
		return null;
	}

	/**
	 * 当收到Opcode.CLOSE时，执行该方法，业务层在该方法中一般不需要写什么逻辑，空着就好
	 * @param wsRequest
	 * @param bytes
	 * @param channelContext
	 * @return 可以是WsResponse、byte[]、ByteBuffer、String或null，如果是null，框架不会回消息
	 * @throws Exception
	 * @author yang
	 */
	@Override
	public Object onClose(WsRequest wsRequest, byte[] bytes, ChannelContext channelContext) throws Exception {
		WsSessionContext wsSessionContext = (WsSessionContext) channelContext.get();
		HttpRequest httpRequest = wsSessionContext.getHandshakeRequest();//获取websocket握手包
		//int count = Tio.getAll(channelContext.tioConfig).getObj().size();

		String name = httpRequest.getParam("name");
		String groupId = httpRequest.getParam("groupId");

		Tio.remove(channelContext, ShowcaseServerAioListener.REMOVE_REMARK);
		Tio.unbindGroup(groupId, channelContext);

		SetWithLock<ChannelContext> byGroup = Tio.getByGroup(channelContext.tioConfig, "123");
		int count = byGroup.getObj().size();


		String msg = "{name:'admin',message:'" + name+ " 离开了，共【" + count + "】人在线" + "'}";
		log.info("{}于{}离开{}群聊",name,simpleDateFormat.format(System.currentTimeMillis()),groupId);
		Tio.remove(channelContext, name+"于"+simpleDateFormat.format(System.currentTimeMillis())+"" +
				"离开群组"+groupId);

		//用tio-websocket，服务器发送到客户端的Packet都是WsResponse
		WsResponse wsResponse = WsResponse.fromText(msg, ShowcaseServerConfig.CHARSET);
		Tio.sendToGroup(channelContext.tioConfig, groupId, wsResponse);
		return null;
	}

	/**
	 * <li>当收到Opcode.TEXT消息时，执行该方法。也就是说如何你的ws是基于TEXT传输的，就会走到这个方法</li>
	 * @param wsRequest
	 * @param text
	 * @param channelContext
	 * @return 可以是WsResponse、byte[]、ByteBuffer、String或null，如果是null，框架不会回消息
	 * @throws Exception
	 * @author yang
	 */
	@Override
	public Object onText(WsRequest wsRequest, String text, ChannelContext channelContext) throws Exception {
		WsSessionContext wsSessionContext = (WsSessionContext) channelContext.get();
		HttpRequest httpRequest = wsSessionContext.getHandshakeRequest();//获取websocket握手包

		String name = httpRequest.getParam("name");
		String groupId = httpRequest.getParam("groupId");


		if (log.isDebugEnabled()) {
			log.debug("握手包:{}", httpRequest);
		}

		log.info("收到ws消息"+name+"于"+simpleDateFormat.format(System.currentTimeMillis())+"发送消息"+":{}", text);

		//客户端会发送心跳包到服务器  tio做服务端来用
		// if (Objects.equals("心跳内容", text)) {
		// 	String msg = "{name:'" + name + "',message:'" + text + "'}";
		// 	//用tio-websocket，服务器发送到客户端的Packet都是WsResponse
		// 	WsResponse wsResponse = WsResponse.fromText(msg, ShowcaseServerConfig.CHARSET);
		// 	//群发
		// 	Tio.sendToGroup(channelContext.tioConfig, groupId, wsResponse);
		// 	return null;
		// }
		//channelContext.getToken()
		//String msg = channelContext.getClientNode().toString() + " 说：" + text;
		String msg = "{name:'" + name + "',message:'" + text + "'}";
		//用tio-websocket，服务器发送到客户端的Packet都是WsResponse
		WsResponse wsResponse = WsResponse.fromText(msg, ShowcaseServerConfig.CHARSET);
		//群发
		Tio.sendToGroup(channelContext.tioConfig, groupId, wsResponse);

		//返回值是要发送给客户端的内容，一般都是返回null
		return null;
	}
}
